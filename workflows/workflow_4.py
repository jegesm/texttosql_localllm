...
# Setup logging
logformat = 'Txt2SQL: %(name)s - %(message)s'
...
class Workflow_4():
    ...
    def __init__(self, sql_llm_model_name, 
        ...
        self.db = SQLDatabase.from_uri(f"postgresql+psycopg2://{self.conn_info['user']}:{self.conn_info['password']}@{self.conn_info['server']}/{self.conn_info['database']}", 
                                  schema=self.conn_info['schema'])
        examples_dict = pd.read_csv(self.examples_path)
